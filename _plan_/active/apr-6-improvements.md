...
for (let i = 0; i < ids.length; i += CHUNK) {
  ...
  const stmts = chunk.map((itemId) =>
    db.insert(itemState)
      ...
      .onConflictDoUpdate({ target: [itemState.itemId, itemState.userId], set: { isRead: 1 } })
  );
...
const tokenRow = await db
  .select({ id: apiTokens.id, userId: apiTokens.userId, email: users.email, lastUsedAt: apiTokens.lastUsedAt })
  .from(apiTokens)
  .innerJoin(users, eq(users.id, apiTokens.userId))
  .where(and(eq(apiTokens.tokenHash, hash), isNull(apiTokens.revokedAt)))
  .get();

if (tokenRow && (!tokenRow.lastUsedAt || Date.now() - tokenRow.lastUsedAt > 3_600_000)) {
